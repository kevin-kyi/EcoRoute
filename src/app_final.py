...
# PITTSBURGH BOUNDING BOX (Updated to include Airport/Suburbs)
    ...
    if not model:
        ...
        with st.status("Thinking...", expanded=True) as status:
            try:
                ...
                try:
                    routes = client.directions(coordinates=[start_coords, end_coords], profile='driving-car', format='geojson', alternative_routes={"target_count": n_routes})
                except:
                ...
                for i, r in enumerate(features):
                    ...
                    candidates.append({
                        ...
                        'geo': r, 'coords': r['geometry']['coordinates']
                    })
                ...
                if current_kwh > needed_kwh:
                    st.session_state.map_layers.append({'geo': best_route['geo'], 'style': {'color': 'blue', 'weight': 5}, 'popup': "Best Route"})
                else:
                    ...
                    if charger:
                        leg1 = client.directions(coordinates=[start_coords, [charger['lon'], charger['lat']]], profile='driving-car', format='geojson')
                        leg2 = client.directions(coordinates=[[charger['lon'], charger['lat']], end_coords], profile='driving-car', format='geojson')
                        ...
                        target_charge_kwh = battery_cap * 0.8 # Charge to 80%
                        ...
                        best_route['leg2'] = {'dist': leg2_sum['distance']/1000, 'time': leg2_sum['duration']/60, 'kwh': leg2_kwh, 'soc': ((target_charge_kwh - leg2_kwh)/battery_cap)*100}
                        best_route['charge_metrics'] = {'added': kwh_added, 'cost': kwh_added * 0.45, 'time': 30} # Assumed 30 min
                        ...
                        st.session_state.map_layers.append({'marker': [charger['lat'], charger['lon']], 'title': charger['title'], 'icon': 'bolt', 'color': 'green'})
                    else:
...
if st.session_state.best_route_stats:
    ...
    if current_kwh > needed_kwh:
        ...
        c1.metric("Distance", f"{stats['dist']:.1f} km")
        c2.metric("Time", f"{stats['time_min']:.0f} min")
        c3.metric("End Battery", f"{((current_kwh - needed_kwh)/battery_cap)*100:.1f}%")
        ...
        with c3:
            ...
            st.metric("Arrival Charge", f"{stats['leg2']['soc']:.1f}%")
            
        # -----------------------------------------------------
        # NEW: FULL TRIP SUMMARY
        # -----------------------------------------------------
        st.divider()
        ...
        sc2.metric("Total Time", f"{total_time:.0f} min", help="Includes 30 min charge")
        sc3.metric("Final Battery", f"{stats['leg2']['soc']:.1f}%")
        sc4.metric("Est. Cost", f"${stats['charge_metrics']['cost']:.2f}", help="Based on $0.45/kWh")